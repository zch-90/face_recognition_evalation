...
#include "../CaffeBinding/CaffeBinding.h"
...
public:
  ...
  FaceAndPoints extract_face(cv::Mat& input_image, vector<Point2d> points, int point_count = 5,
                             ...
                             float max_shear_ratio = 0, float max_aspect_ratio = 0.1, float max_rotate_angle = 10,
    ...
    for (int i = 0; i < lines.size(); i++) {
      ...
      for (int f = 0; f < patch_per_face; f++) {
        ...
        do {
          ...
          for (int p = 0; p < 5; ++p) {
            if (image_points.points[p].x < -0.48 * (double)image_points.image.cols ||
                image_points.points[p].x > 0.48 * (double)image_points.image.cols ||
                image_points.points[p].y < -0.48 * (double)image_points.image.rows ||
                image_points.points[p].y > 0.48 * (double)image_points.image.rows) {
              valid_sample = false;
            }
          }
        ...
        label.push_back(1);
        for (int p = 0; p < 5; ++p) {
          label[p + 2] = image_points.points[p].x;
          label[p + 3] = image_points.points[p].y;
        }
